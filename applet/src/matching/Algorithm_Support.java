...
public interface Algorithm_Support
{
	...
	float input_initial_disc_size(short node_number);
	...
	// algorithms should call this once each time through step to indicate
	// whether

	// they are in the process of augmenting

	float input_distance(short node_number_1, short node_number_2);

	// the algorithm should use this function to find the real distance between
	// points as

	// this will more easily allow for future enhancements involving different
	// metrics
	...
	// to inform the rest of the program whether or not it's waiting for a new
	// node
	...
	// an algorithm should call this once, when finnished, but awaiting a
	// post_opt call
	...
	void output_change_by(float distance);
	...
	// -- instead, bipartite algorithms should call
	// algorithm_output_is_node_blue
