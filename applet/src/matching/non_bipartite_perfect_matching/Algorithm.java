...
public class Algorithm implements Algorithms_Provide
{
	...
	public boolean step_post_op(short node_number)
	{
		...
		if (VIRTUAL_NODE_NUMBER == node_number)
		{
			...
			if (action_to_be_taken == MAKE_NEW_BLOSSOM)
			{
				...
				support.output_remove_tight_line((first_affected_blossom).node_number,
						(second_affected_blossom).node_number);
		...
		if (null == node_we_are_opting_on)
		{
			...
			// distinguishing feature is that its node number is
			// VIRTUAL_NODE_NUMBER
		...
		// ask it to grow tree starting from virtual node. Be sure to disolve
		// any
	...
	public boolean step(short node_number)
	{
		...
		boolean dont_show_next_iteration; // when true, we are doing a
		// degenerate tree grow
		...
		case DISSOLVE_BLOSSOM:
			...
			// we don't to continue on through the rest of the function and do
			// the next bump
		...
		case MAKE_NEW_BLOSSOM:
			...
			// we want to continue on through the rest of the function and do
			// the next bump
		...
		case AUGMENT:
			...
			// there was no item to do in the augment queue
			{
			...
			// the augment queue is now empty
			{
			...
			// we DON'T want to continue on through this function when
			// augmenting
		...
		{
			// the tree has only one expanding moat and no nodes are outside of
			// the tree
		...
		// modify our disks radius's and tell the rest of the program what to
		// shrink and grow
		...
		case MAKE_NEW_BLOSSOM:
			...
			// nodes located in outermost blossoms in the tree which are
			// colliding

			// add the tight line where the two nodes in expanding blossoms
			// collided
		...
		case ADD_TO_TREE:
			...
			// it's a normal grow
			{
				...
				support.output_add_tight_line((first_affected_blossom).node_number,
						(second_affected_blossom).node_number);

				// find_next_bump modified some of our global variables:
				// first_affected_blossom is now the node not in the tree which
				// is
				// bumped by the node second_affected_blossom in the tree

				// add_blossom_to_tree will add the outermost blossom containing
				// the
				// node first_affected_blossom to the outermost blossom in the
				// tree
		...
		case DISSOLVE_BLOSSOM:
			...
			// do nothing special at this point, just continue on with the rest
			// of the function
		...
		case NOTHING:
			...
			{
				// do nothing probably a disc went to zero and we are just
				// pausing
			}
		...
		if (ZERO_SHOW)
		{
			...
			// we need to remember the previous action, because we really want
			// to do the
			// action that find_next_bump suggests in the following test, only
			// if it
			// actually is a zero growth iteration.

			while (!support.input_is_zero_growth_being_shown() && ADD_TO_TREE == action_to_be_taken
	...
	private float find_next_bump()
	{
		...
		//
		// -- any node in an expanding outermost blossom colliding with any node
		// in another
		// expanding outermost blossom
		// -- any node in an expanding outermost blossom colliding with any node
		// in an
		// outermost blossom which isn't in the tree (so it's neither expanding
		// or shrinking)
		...
		for (current_blossom = first_blossom; null != current_blossom; current_blossom = (current_blossom).this_is_contained_with_me)
		{
			...
			if (current_blossom.is_a_node() && VIRTUAL_NODE_NUMBER == (current_blossom).node_number)
				continue; // we do not want to compare the virtual node with
			// anything!
			...
			if ((current_blossom).i_am_in_tree)
			{
				...
				if ((current_blossom).i_am_expanding)
				{
					...
					// look at all the outermost blossoms to the "right" which
					// are not in tree or are expanding
					...
					for (temp_blossom = (current_blossom).this_is_contained_with_me; null != temp_blossom; temp_blossom = (temp_blossom).this_is_contained_with_me)
					{
						...
						if ((temp_blossom).i_am_in_tree)
						{
							...
							if ((temp_blossom).i_am_expanding)
							{
								...
								distance_to_change = compare_blossoms(current_blossom, temp_blossom,
										distance_to_change, true);
						...
						// temp_blossom is not in tree
						{

							// notice we make sure the blossom out of the tree
							// is always first
							// --this is so that later we will know which one to
							// add to the tree

							// System.out.println("in find_next_bump" +
							// distance_to_change);
							distance_to_change = compare_blossoms(temp_blossom, current_blossom, distance_to_change,
									false);
				...
				// current_blossom is shrinking
				{

					// check to see of the current_blossom is not a trivial
					// blossom
					// -- because nodes can go negative in non-triangle
					// inequality metrics
					...
					{
						// it is a node, and it is about to go negative, then do
						// nothing so we get a pause for visual effect.
				...
				for (temp_blossom = (current_blossom).this_is_contained_with_me; null != temp_blossom; temp_blossom = (temp_blossom).this_is_contained_with_me)
				{
					...
					if ((temp_blossom).i_am_in_tree)
					{
						...
						if ((temp_blossom).i_am_expanding)
						{
							...
							distance_to_change = compare_blossoms(current_blossom, temp_blossom, distance_to_change,
									false);
			...
			boolean are_both_expanding)
	{
		...
		while (true)
		{
			...
			while (true)
			{
				...
				// System.out.println("in compare_blossoms, temp_distance " +
				// temp_distance);
				// System.out.println("in compare_blossoms, distance_to_change "
				// + distance_to_change);
				...
				if (temp_distance < distance_to_change)
				{
					...
					// elsewhere, when we called this function, we made sure
					// that if one of
					// the blossoms was not in the tree, it was the first
					// argument
					// we preserve this order now so that later we will know
					// that
					// (if applicable) first_affected_blossom is the one to add
					// to the tree
		...
		} // end while we have found a new temp_node_one to compare with all of
		// blossom_two
	...
	private void make_new_blossom()
	{
		...
		blossom_in_tree_where_their_paths_meet = find_meeting_outermost_blossom(
				outermost_blossom_containing_first_affected_node, outermost_blossom_containing_second_affected_node);

		current_colour++;

		add_connection_in_blossom(new_blossom, get_spouse_node_of(blossom_in_tree_where_their_paths_meet), true);
		remove_connection_in_blossom(blossom_in_tree_where_their_paths_meet,
				get_spouse_node_of(blossom_in_tree_where_their_paths_meet));

		// deal with the two affected blossoms first, unless one of them is the
		// actually the
		// blossom in the tree where the two paths meet, in which case we want
		// to leave
		...
		if (outermost_blossom_containing_first_affected_node != blossom_in_tree_where_their_paths_meet)
		{
			...
			// we need to get save this handle before we modify temp_node
			// because
			// modifying it will ruin our chances of getting any information out
			// of it
			// -- next_outermost_blossom_towards_tree_root_from relies on the
			// fact that
			// the given blossom and the sought after blossom will be contained
			// in the same blossom -- this won't be true once we move
			// temp_blossom into new_blossom
			...
			// now go around the first arm of the tree, removing the blossoms
			// from being
			...
			while (null != temp_blossom && temp_blossom != blossom_in_tree_where_their_paths_meet)
			{
				...
				if ((temp_blossom).i_am_expanding)
				{
					...
					add_connection_in_blossom(temp_blossom,
							(previous_blossom).node_in_me_connected_out_towards_tree_root_node, false);

				}

				// else, if temp_blossom is shrinking, there is no need to add a
				// new connection down the tre,
				// it should already be there, because we added the connection
				// to when we added
			...
			add_connection_in_blossom(blossom_in_tree_where_their_paths_meet,
					(in_first_arm_before_meeting_place).node_in_me_connected_out_towards_tree_root_node, false);
		...
		if (outermost_blossom_containing_second_affected_node != blossom_in_tree_where_their_paths_meet)
		{
			...
			add_connection_in_blossom(blossom_in_tree_where_their_paths_meet,
					(in_second_arm_before_meeting_place).node_in_me_connected_out_towards_tree_root_node, false);
		...
		// we don't need to worry about the way the new blossom interacts with
		// outermost blossoms
		// in the tree that are related to constituents of the new blossom
		// because
		// the way we stored these relations -- through the specific node to
		// which a blossom
	...
	private void dissolve_blossom()
	{
		...
		// -- first_affected_blossom contains the blossom who's width has shrunk
		// to 0
		...
		first_blossom = find_containing_blossom_which_is_inside_given_blossom(
				(blossom_to_dissolve).node_in_me_connected_out_towards_tree_root_node, blossom_to_dissolve);

		last_blossom = find_containing_blossom_which_is_inside_given_blossom(
				get_node_in_me_which_is_married_out(blossom_to_dissolve), blossom_to_dissolve);

		// we have to check whether or not in blossom_to_dissolve the tree jumps
		// into the
		...
		if (first_blossom == last_blossom)
		{
			...
			// special case when tree jumps into blossom_to_dissolve in same
			// place it jumps out
			// all the blossoms but that one are booted out of the tree

			// just pick one way to go around the cycle -- get_non_spouse_of
			...
			do
			{
				...
				if (odd_number_of_blossoms_away_from_last_blossom)
				{
					...
					support.output_remove_tight_line((get_node_in_me_which_is_tight_out(current_blossom)).node_number,
							(temp_node_in_next_which_is_tight_out).node_number);

				}
				else
				// even number of blossoms away from spouse
				{

					// we want to be careful to save things first before we
					// modify a blossom

					next_blossom = get_spouse_of(current_blossom);

					// the function get_node_in_me_which_is_tight_out won't work
					// anymore
					// because we just came from disconnecting the blossom to
					// which
					// the current_node is tight -- we use
					// temp_node_in_next_which_is_tight_out
					// which the last blossom left for us

					support.output_remove_tight_line((temp_node_in_next_which_is_tight_out).node_number,
							(get_non_spouse_node_of(current_blossom)).node_number);
				...
				// put back into the pool of outermost blossoms as not in the
				// tree
			...
			// last_blossom will need to take over some information about the
			// tree structure from blossom_to_dissolve
			...
			// now remove what used to be the second non-spouse connection of
			// last blossom
		...
		// the tree passes through more than one blossom of blossom_to_dissolve
		{

			// we need to start at the place where the tree comes into
			// blossom_to_dissolve
			// and make an augmenting path of blossoms which will remain in the
			// tree
			...
			do
			{
				...
				// it should no longer be in blossom_to_dissolve, and should be
				// put back into the pool of outermost blossoms as part of the
				// tree
			...
			// special case when the blossom only has three blossoms in it -- we
			// will
			// never have had a chance to make the display of the tight line
			// between
			...
			// doing the intervening blossoms that we remove the display of the
			// obsolete
			// tight lines
			...
			if (last_blossom == get_non_spouse_of(first_blossom))
			{
				...
				support.output_remove_tight_line((get_node_in_me_which_is_tight_out(first_blossom)).node_number,
						(get_non_spouse_node_of(first_blossom)).node_number);
			...
			// first_blossom will need to take over some information about the
			// tree structure from blossom_to_dissolve
			...
			// remove the tight connection to the arm of the blossom which will
			// get left out
			// of the tree -- current_blossom is still pointing to the first
			// blossom in
			// that direction -- we use the fact that current_blossom is now an
			// outermost blossom
			// to figure out which of the two non-spouse connections in
			// last_blossom needs
			// to be disconnected
			...
			// we want to remove the second connection
			{

				remove_connection_in_blossom(last_blossom,
						(last_blossom).second_connection.node_in_other_to_which_i_am_connected);
	...
	private void augment()
	{
		...
		// augmentation through the outermost blossoms is ALMOST like
		// augmentation
		...
		// so take care of the augmentation between the node in the tree and the
		// node
		// out of the tree which bumped right now -- but don't add the new
		// spouse to
		// the node in the tree yet -- leave it alone so that
		// recursive_subroutine_augment_blossom
		// will know the correct spouse to go down the tree -- just add the new
		// spouse
		...
		if (!bumped_into_infinity)
		{
			...
			// note that we know that the node out of the tree which bumped is
			// an outermost blossom
			...
			// we can SHOW the marriage between the two nodes which bumed, even
			// if we
		...
		// now work our way through the outermost blossoms from the node in the
		// tree
		...
		// -- passing null to recursive_subroutine_augment_blossom is our way of
		// saying that
		...
		add_connection_in_blossom(find_outermost_blossom_containing(second_affected_blossom), first_affected_blossom,
				true);
	...
	private void recursive_subroutine_augment_blossom(blossom blossom_to_augment, blossom from_node, blossom to_node)
	{
		...
		if (null == blossom_to_augment)
		{
			...
			// we are augmenting through the outermost blossoms in the tree ,
			// which is
		...
		if (first_blossom_to_augment == last_blossom_to_augment)
		{
			...
			// the tree jumped into the same blossom in blossom_to_augment as it
			// jumped out of
		...
		// if we got to here, we know we are supposed to do some augmenting in
		// this blossom

		// now, if (in blossom_to_augment) the blossom the tree jumped into
		// isn't the
		// same as the blossom the tree jumped out of, we know from the
		// structure of
		// blossoms that the blossoms in blossom_to_augment will come in pairs:
		// -- first_blossom_to_augment will be married to the next one toward
		// the tree root,
		// then that one will be tight with the next one, then married, then
		// tight,
		...
		// another_temp_spouse_node = get_node_in_me_which_is_married_out(
		// current_blossom );

		our_augment_queue.put_new_augment_item_into_queue((temp_spouse_node).node_number,
				(get_node_in_me_which_is_married_out(current_blossom)).node_number, false);

		recursive_subroutine_augment_blossom(current_blossom, from_node,
				get_node_in_me_which_is_married_out(current_blossom));
		...
		while (next_blossom != last_blossom_to_augment)
		{
			...
			if (odd_number_of_blossoms_away_from_last_blossom)
			{
				...
				// save this right away because we are going to screw up current
				// blossom's marital status
				...
				if (we_are_augmenting_at_outermost_level)
				{
					...
					// we are in an outermost blossom, so we can't rely on the
					// function
					// get_node_in_me_which_is_tight_out to work because it
					// depends
					// on the fact that the node i am tight with is also tight
					// with me,
					// which is not the case in the outermost blossom tree, so
					// use
					// the special field we have just to make up for this
					// problem
				...
				// we are not in an outermost blossom
				{
				...
				our_augment_queue.put_new_augment_item_into_queue((temp_non_spouse_node).node_number,
						(temp_node_in_me_which_is_tight_out).node_number, true);

				// when calling recursive_subroutine_augment_blossom, we must
				// always remember to make sure that
				// the from_node is NOT the node in the blossom which is married
				// out -- this is the node
				// which will have no spouse in the blossom, making the first
				// step in the augmentation
				// of that blossom (which is to go towards the spouse)
				// impossible

				recursive_subroutine_augment_blossom(current_blossom, temp_node_in_me_which_is_tight_out,
						node_in_me_which_connects_me_away_from_tree_root);
				...
				// we change this pointer so that the tight edge out of current
				// blossom
			...
			// we are an even number of blossoms away from
			// last_blossom_to_augment
			{
				...
				if (we_are_augmenting_at_outermost_level)
				{
					...
					// we are in an outermost blossom, so we can't rely on the
					// cycle
					// of spouse, non-spouse to work -- this blossom (the even
					// one which
					// is about to become the current_blossom, but not yet) will
					// in
					// fact have no non-spouse, so we must use the special
					// field of the current_blossom used in trees for this
					// purpose
				...
				recursive_subroutine_augment_blossom(current_blossom, node_in_me_which_connects_me_away_from_tree_root,
						get_node_in_me_which_is_married_out(current_blossom));
				...
				// remove_connection_in_blossom( next_blossom,
				// another_temp_spouse_node );
				// add_non_spouse_to_blossom( next_blossom,
				// another_temp_spouse_node );
		...
		// augment the last blossom in blossom_to_augment, passing it
		// blossom_to_augment's to_node

		recursive_subroutine_augment_blossom(last_blossom_to_augment, node_in_me_which_connects_me_away_from_tree_root,
				to_node);

		// in what follows, we have to check for the special case when the
		// actual tree_root is involved
		...
		if (we_are_augmenting_at_outermost_level)
		{
			...
			// an outermost blossom won't have ever had any connections (usually
			// base blossoms in blossoms
			// have two non-married connections we can check for) so to figure
			// out what this last
			// blossom must marry, we use the temp_node_in_me_which_is_tight_out
			// in the second last blossom -- which current_blossom was left
			// pointing to
		...
		// the last_blossom_to_augment is not an outermost blossom
		{

			// since last_blossom_to_augment is not outermost, it is in a
			// blossom, so we know that
			// it will have two connections, unfortunately both are non-spouse,
			// so we have to check
			// to see which one of it's connections connects it to the second
			// last
	...
	private void shrink_and_grow_the_tree()
	{
		...
		for (temp_outermost_blossom = first_blossom; null != temp_outermost_blossom; temp_outermost_blossom = (temp_outermost_blossom).this_is_contained_with_me)
		{
			...
			if ((temp_outermost_blossom).i_am_in_tree)
			{
				...
				// the blossom is not just a node
				{

					recursive_change_all_nodes_in_blossom((temp_outermost_blossom).i_contain,
							distance_by_which_to_change, (temp_outermost_blossom).i_am_expanding);
	...
	private boolean set_up_new_tree_root_node(short node_number)
	{
		...
		// we were given a node_number to use, let's see if it's valid
		{
	...
	private void recursive_change_all_nodes_in_blossom(blossom the_blossom, float distance, boolean expand_it)
	{
		...
		if (the_blossom.is_a_node())
		{
			...
			// the node is in a shrinking blossom
			{
			...
			blossom given_blossom)
	{
		...
		if ((contained_blossom).i_am_contained_in == given_blossom)
		{
			...
			// we have found the containing blossom which is inside the given
			// blossom
		...
		// we are not at the outermost blossom yet
		{

			return (find_containing_blossom_which_is_inside_given_blossom((contained_blossom).i_am_contained_in,
					given_blossom));
			...
			boolean make_it_a_marriage)
	{
		...
		// #if 0 // old stuff from before implementation of maximal minimum
		// weight pm
		//
		// if( null == me || null ==
		// the_node_in_the_other_to_which_i_am_connected )
		//
		// #else
		...
		// #if DEBUGGING
		//
		// else // both connections were already used -- this is bad and
		// shouldn't have happened
		// {
		//	
		// // error detection -- we have a blossom that should have had an
		// unused connection but didn't
		//
		// printf(
		// "You just tried to connect something to a blossom which already has two connections\n");
		//	
		// }
		//
		// #endif
	...
	private void remove_connection_in_blossom(blossom me, blossom the_node_in_the_other_to_which_i_am_connected)
	{
		...
		// #if DEBUGGING
		//
		// else // the node wasn't connected that way in the first place -- this
		// is bad and shouldn't have happened
		// {
		//	
		// // error detection -- somewhere, we thought something was
		// connectected when it wasn't
		//
		// printf(
		// "You just tried to disconnect two blossoms that weren't connected\n");
		// }
		//
		// #endif
	...
	private blossom get_spouse_of(blossom the_blossom)
	{
		...
		{
			temp = find_containing_blossom_which_is_inside_given_blossom(
					(the_blossom).first_connection.node_in_other_to_which_i_am_connected,
					(the_blossom).i_am_contained_in);
			return (temp);
			// return( find_containing_blossom_which_is_inside_given_blossom(
			// (the_blossom).first_connection.node_in_other_to_which_i_am_connected,
			// (the_blossom).i_am_contained_in ) );
		...
		else if ((the_blossom).second_connection.is_a_marriage)
		{
			...
			temp = find_containing_blossom_which_is_inside_given_blossom(
					(the_blossom).second_connection.node_in_other_to_which_i_am_connected,
					(the_blossom).i_am_contained_in);
			return (temp);
			// return( find_containing_blossom_which_is_inside_given_blossom(
			// (the_blossom).second_connection.node_in_other_to_which_i_am_connected,
	...
	private blossom get_non_spouse_of(blossom the_blossom)
	{
		...
		return (find_containing_blossom_which_is_inside_given_blossom(
				(the_blossom).second_connection.node_in_other_to_which_i_am_connected, (the_blossom).i_am_contained_in));
	...
	private void add_node()
	{
		...
		temp_node = first_blossom;
		// if( ! temp_node.is_a_node() || ! VIRTUAL_NODE_NUMBER ==
		// (temp_node).node_number )
		// {
	...
	private blossom find_eligible_node_with_number(short node_number)
	{
		...
		for (temp_node = first_blossom; null != temp_node; temp_node = (temp_node).this_is_contained_with_me)
		{
			...
			if ((temp_node).node_number == node_number)
			{
				...
				// it is the node we want, but it is no longer eligible
				{
	...
	private void make_blossom_not_be_contained_in_anything(blossom the_blossom)
	{
		...
		if (null == (the_blossom).i_am_contained_in)
		{
			...
			// the blossom to be removed isn't the first blossom in the tree
			{
				...
				for (temp_blossom = first_blossom; null != (temp_blossom).this_is_contained_with_me
						&& the_blossom != (temp_blossom).this_is_contained_with_me; temp_blossom = (temp_blossom).this_is_contained_with_me)
				{
					// do nothing, just move forward through the list of
					// blossoms
				}
		...
		// the blossom we are removing isn't an outermost blossom
		{
			...
			if ((containing_blossom).i_contain == the_blossom)
			{
				...
				// the_blossom is the first in the list of blossoms belonging to
				// the
				// containing blossom -- repair the linked list, making it skip
				// over the_blossom
			...
			// the blossom isn't the first in any list of blossoms
			{

				for (temp_blossom = (containing_blossom).i_contain; null != (temp_blossom).this_is_contained_with_me
						&& the_blossom != (temp_blossom).this_is_contained_with_me; temp_blossom = (temp_blossom).this_is_contained_with_me)
				{
					// do nothing, just move forward through the list of
					// blossoms
			...
			blossom outermost_blossom_in_tree_two)
	{
		...
		for (temp_blossom = outermost_blossom_in_tree_two; null != temp_blossom; temp_blossom = next_outermost_blossom_towards_tree_root_from(temp_blossom))
		{
			...
			if (!(temp_blossom).i_am_in_tree)
			{
				...
				// return_value to all the rest of the blossoms down the line
				// which
	...
	private void make_blossom_be_contained(blossom blossom_to_be_contained, blossom containing_blossom)
	{
		...
		// add the blossom to be contained into the containing blossom first --
		// order
		// of containment doesn't mean anything, and it's easier to do it this
		// way

		// note: any blossom_to_be_contained's this_is_contained_with_me field
		// is overwriten
		// and lost, but there should never be a context where it is meaningful
		// as
		// a node should always have just been removed from another blossom
		// before it is made
		// to be contained in another one -- blossoms are always removed one at
		// a time so
		...
		if (null == containing_blossom)
		{
			...
			// special case, we are being told to add the blossom to the list of
			// outermost blossoms
			...
			{
				// we will add it as the second in list as the first is a
				// virtual node
		...
		// we are just adding a blossom to an ordinary blossom
		{
	...
	private blossom get_spouse_node_of(blossom the_blossom)
	{
		...
		// the blossom doesn't have a spouse
		{
	...
	private void reset_tree()
	{
		...
		for (temp_blossom = first_blossom; null != temp_blossom; temp_blossom = (temp_blossom).this_is_contained_with_me)
		{
			...
			if ((temp_blossom).i_am_in_tree)
			{
				...
				// now remove any non-spouse tightness between blossoms which
				// are at the outermost level
				// -- only blossoms (and nodes) within other blossoms retain
				// their tight lines when
				// the tree is reset. Only shringing blossoms need be looked at.

				if (!(temp_blossom).i_am_expanding
						&& null != (another_temp_node = get_non_spouse_node_of(temp_blossom)))
				{
					...
					// we are about to access some handles which ought to be
					// non-null
					// we won't check it because if it is null, then something
					// is wrong with
					// the algorithm somewhere else and this way we'll find out
					// about it pretty fast

					support.output_remove_tight_line(
							((temp_blossom).node_in_me_connected_out_towards_tree_root_node).node_number,
							(another_temp_node).node_number);
					// support.output_remove_tight_line(
					// (get_non_spouse_node_of(find_outermost_blossom_containing(another_temp_node))).node_number,
					// (another_temp_node).node_number );

				}

				// other tree related variables like i_am_expanding and
				// node_in_me_connected_out_towards_tree_root_node
				// shouldn't trouble us because they are ignored unless a
				// blossom is put in a tree
				// again and when that happens those variables will be set to
				// new proper values anyway
		...
		// #if 0
		// // experimental, we reset EVERY blossom, just to make sure of
		// cleanliness
		...
		// while( true )
		// {
		// while( temp_blossom != null && null ==
		// (temp_blossom).this_is_contained_with_me )
		// {
	...
	private boolean is_single(blossom the_node)
	{
		...
		// the node is contained in some blossom, so it must be married
		{

			// note: the tree root node can be in a blossom while at the same
			// time
			// being single, but we won't ever be asking whether the
			// tree_root_node
			// is single or not
	...
	private blossom get_node_in_me_which_is_tight_out(blossom the_blossom)
	{
		...
		// the non_spouse_blossom itself had more than one non-spouses, and the
		// first one wasn't the correct one
		{
	...
	private void add_discs_to_all_nodes_in_blossom(blossom the_blossom)
	{
		...
		// is a node
		// -- we want to use recursion but can't right away because we don't
		// want
		// to go off and add_discs to everything down the right side of
		// the_blossom
		// i.e. we have to ignore stuff in the this_is_contained_with_me field
		// the first time
	...
	private blossom find_node_not_in_tree_with_biggest_radius()
	{
		...
		for (temp_outermost_blossom = first_blossom; null != temp_outermost_blossom; temp_outermost_blossom = (temp_outermost_blossom).this_is_contained_with_me)
		{
			...
			if (!(temp_outermost_blossom).i_am_in_tree)
				widest_node_so_far = recursive_subroutine_find_node_in_blossom_with_biggest_radius(
						temp_outermost_blossom, widest_node_so_far);
			...
			blossom widest_node_so_far)
	{
		...
		if (null == (the_blossom).i_contain)
		{
			...
			// it's just a node, compare and recurse to next blossom contained
			// with
		...
		// its a blossom, recurse inward
		{

			widest_node_so_far = recursive_subroutine_find_node_in_blossom_with_biggest_radius((the_blossom).i_contain,
					widest_node_so_far);
		}

		return (recursive_subroutine_find_node_in_blossom_with_biggest_radius((the_blossom).this_is_contained_with_me,
				widest_node_so_far));
	...
	//
	// // although it shouldn't matter to the program, because it should be
	// ignoring this
	// // value at this point, print_memory_dump will try to print out the
	// blossom in
	// // first_affected_blossom which will be garbage after dissolve blossom
	// has nuked it
	//
	// first_affected_blossom = null;
	//
	...
	//	
	// if( null != (the_blossom).i_am_contained_in )
	// printf( "   i am contained in: %4d",
	// ((the_blossom).i_am_contained_in).node_number );
	// else
	...
	// if( null !=
	// (the_blossom).first_connection.node_in_other_to_which_i_am_connected )
	// printf( "   first connection: %4d",
	// ((the_blossom).first_connection.node_in_other_to_which_i_am_connected).node_number
	// );
	// else
	...
	// if( null !=
	// (the_blossom).second_connection.node_in_other_to_which_i_am_connected )
	// printf( "   second connection: %4d",
	// ((the_blossom).second_connection.node_in_other_to_which_i_am_connected).node_number
	// );
	// else
