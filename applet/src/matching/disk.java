...

{
	...
	/** algorithms which need moats must have the disks surrounding each node drawn in a particular order, 
		otherwise the moats won't look right.  this method uses a class variable to draw all
		the disks in the correct order */
	...
	/** forces garbage collection of the static variable next_disk_to_be_drawn, which
		otherwise will continue to point to the disk objects, keeping them from being
	...
	/** repairs linked list of disks which are in correct order to be drawn
		so that when this disk is removed, the linked list will be
		intact without it */
	void prepare_to_remove()
	{
		...
		{
			for(previous_disk = first_disk_to_be_drawn, temp_disk = first_disk_to_be_drawn.next_disk_to_be_drawn;
				temp_disk != null; previous_disk = temp_disk, temp_disk = temp_disk.next_disk_to_be_drawn )
			{
				if( temp_disk == this )
				{
					// we've found it
		...
		//if we made it to here, then the disk wasn't in the linked list which is actually
...

	{
		...
		g.fillOval( Math.round( my_node.x - radius), Math.round(my_node.y - radius), Math.round(radius*2), Math.round(radius*2) );
		...
		//g.fillOval((int)(my_node.x - radius)+1, (int)(my_node.y - radius)+1, (int)(radius*2)-2, (int)(radius*2)-2 );
		g.fillOval(Math.round(my_node.x - radius), Math.round(my_node.y - radius), Math.round(radius*2), Math.round(radius*2) );
		...
		if( radius >= 0 )
		{
			g.drawOval(Math.round(my_node.x - radius), Math.round(my_node.y - radius), Math.round(radius*2), Math.round(radius*2) );
		}
		else
		{
			g.drawOval(Math.round(my_node.x + radius), Math.round(my_node.y + radius), Math.round(-radius*2), Math.round(-radius*2) );			
