...
class Nonbipartite_Spanning_Tree implements Algorithms_Provide
{

	NSTNode first_node;

	NSTNode group_needing_moat;

	NSTNode bumper_one;
	NSTNode bumper_ones_group;
	NSTNode bumper_two;
	NSTNode bumper_twos_group;

	double nearest_distance_so_far;

	short current_colour;
...
Nonbipartite_Spanning_Tree( Algorithm_Support the_support, short number_of_nodes )
{
...
public boolean	step( short node_number)
{
	...
	// ignore the node_number we are passed, it has no meaning for this algorithm
	...
	nearest_distance_so_far = 10000000.0;
	...
	if( null != bumper_one && null != bumper_two )
	{
		...
		else	// it's not the first time we've been called
		{
		...
		support.output_change_by( nearest_distance_so_far / 2.0 );
...
private void find_next_bump()
{
	...
	for( temp_top_level = first_node; null != temp_top_level; temp_top_level = temp_top_level.next_on_top_level )
	{
		...
		// we want to compare everything in a group with everything not in that group

		for( temp_node_in_group = temp_top_level; null != temp_node_in_group; temp_node_in_group = temp_node_in_group.next_in_same_group )
		{
		
			// we only want to compare with everything further than us on the top level,
			// 	because everything before us has already been checked against us
...
private void compare_with_everything_else( NSTNode the_group_we_are_checking, NSTNode the_node_we_are_checking, NSTNode the_start_of_everything_else)
{
	...
	if( null == the_node_we_are_checking || null == the_start_of_everything_else || null == the_group_we_are_checking)
	{
		return;
	}
	...
	for( temp_top_level = the_start_of_everything_else; null != temp_top_level; temp_top_level = temp_top_level.next_on_top_level )
	{
		...
		for( temp_node_in_group = temp_top_level; null != temp_node_in_group; temp_node_in_group = temp_node_in_group.next_in_same_group )
		{
			...
			//System.out.println("in compare " + "node1 " + the_node_we_are_checking.node_number + " node2 " + temp_node_in_group.node_number);

			if( (temp_distance = support.input_distance( the_node_we_are_checking.node_number, temp_node_in_group.node_number )
...
private void consolidate_groups( NSTNode group_one, NSTNode group_two)
{
	...
	// we will leave group_one alone, and just insert group_two into group_one,
	...
	if( group_two == first_node )
	{
		...
		first_node = group_two.next_on_top_level;	// we know that this is o.k. because of safety checks above
	
	}
	else	// group_two is not first_node, so we must find the top_level node just before
	...
	// insert group_two into group_one, just after group_one's top_level node
	...
	// 	-- loop condition looks risky but isn't because we know group_two exists
